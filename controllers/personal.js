...
var Personal = require('../models/datospersonales');
...
function saveInfo(req, res){
	...
	personal.perfil = params.perfil;
	personal.telefono = params.telefono;
	personal.curriculum = params.curriculum;
	personal.pais =params.pais;
	personal.user = req.user.sub;
	personal.name = params.name;
	personal.surname = params.surname;
	personal.estudytipe = params.studytipe;
	

	personal.save((err, personalStored) => {
		if(err) return res.status(500).send({message: 'Error en la Informacion'});

		if(!personalStored) return res.status(404).send({message: 'La Informacion NO ha sido guardada'});
